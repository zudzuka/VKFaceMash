...
def download(self): # Download library
    ...
    for i in range(len(profiles['items'])):
        self.append(Person())
        self[i].name    = profiles['items'][i]['first_name']
        self[i].sname = profiles['items'][i]['last_name']
        self[i].id  = profiles['items'][i]['id']
        self[i].jpg = r'pics/'+str(profiles['items'][i]['id'])+'.jpg'
